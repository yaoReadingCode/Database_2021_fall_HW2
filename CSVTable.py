...
class CSVTable:
    ...
    def __add_row__(self, row):
        ...
        if defined_indexes is not None:
            for index in defined_indexes:
                ...
                key_string = self.__get_key__(index, row)   # returns a string that is the concatentated version for the index
    ...
    def __get_access_path__(self, fields):
        ...
        # ************************ TO DO ***************************
        if len(self.__indexes__) == 0 or fields is None:
            return None, None
        
        most_selective_index = None
        count = None
        for index_name in self.__indexes__.keys():
            column_names = index_name.split('_')
            tag = False
            ...
            if most_selective_index is None:
                most_selective_index = index_name
                count = len(self.__indexes__[index_name])
            else:
                if len(self.__indexes__[index_name]) > len(most_selective_index):
                    most_selective_index = index_name
                    count = len(self.__indexes__[index_name])
        
        if most_selective_index is None:
            return None, None
        return most_selective_index, count
    ...
    def __find_by_template_scan__(self, t, fields=None):
        ...
        for row in self.__rows__:
            if self.matches_template(row, t):
                projected_r = self.project([row], fields)[0]
                result.append(projected_r)
        return result
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        # ************************ TO DO ***************************
        result = []
        key_string = '_'.join(t.values())
        rows = self.__indexes__[idx_name][key_string]
        for row in rows:
            projected_r = self.project([row], fields)[0]
            result.append(projected_r)
        return result
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        # ************************ TO DO ***************************
        left_r = self
        left_sub_templates = left_r.__get_sub_where_template__(where_template)
        right_sub_templates = right_r.__get_sub_where_template__(where_template)

        left_result = left_r.__find_by_template__(template=left_sub_templates)
        right_result = right_r.__find_by_template__(template=right_sub_templates)

        new_left = self.__table_from_rows__("Filtered (" + left_r.__table_name__ + ")", left_result)
        new_right = self.__table_from_rows__("Filtered (" + right_r.__table_name__ + ")", right_result)

        left_rows = new_left.__get_row_list__()
        right_rows = new_right.__get_row_list__()
        ...
        join_result = self.__table_from_rows__("JOIN:" + left_r.__table_name__ + ":" + right_r.__table_name__, result_rows)
        result = join_result.__find_by_template__(template=None, fields=project_fields)
        result_table = self.__table_from_rows__("JOIN:" + left_r.__table_name__ + ":" + right_r.__table_name__, result)
        return result_table
